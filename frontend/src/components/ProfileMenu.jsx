...
function ProfileMenu() {
  ...
  useEffect(() => {
    profileDropdown.current.classList.toggle("-rotate-180");
  }, [isMenuOpen]);
  ...
  return (
    <Menu animate={{
        mount: { y: 0 },
        unmount: { y: -25 },
      }} open={isMenuOpen} handler={setIsMenuOpen} >
      {/* <div onClick={handleProfileMenu} className="w-20 bg-red-500 absolute h-14 rounded-full z-10">
        
      </div> */}
      <MenuHandler >
        <div 
          className="flex md:w-20 items-center justify-between cursor-pointer z-10 rounded-full hover:bg-gray-300 p-1 active:bg-gray-400" >
          <Avatar
            src="https://avatars.githubusercontent.com/u/58645688?v=4"
            alt="avatar"
            className="border-light-secondary border-[1px] rounded-full p-0.5 select-none"
          />
          <span
            ref={profileDropdown}
